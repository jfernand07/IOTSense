...
import { AlertSeverity } from '../module/alerts/entities/alert.entity';
...
export class AiService {
  ...
  private mapSeverity(level: number): AlertSeverity {
    if (level >= 4) return AlertSeverity.CRITICAL;
    if (level === 3) return AlertSeverity.WARNING;
...
`;
      ...
      if (resultadoFinal.anomalia && data.plantId) {
        const alertDto: CreateAlertDto = {
          ...
          severity: this.mapSeverity(primeraRespuesta.severidad),
